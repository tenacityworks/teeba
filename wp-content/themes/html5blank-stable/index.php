<?php get_header(); ?>
	<div class="container" style="padding-top: 200px; padding-bottom: 120px;">
		<h1 class="text-center title">Your Health is the World</h1>
			<div class="separator"></div>
				<p style=" font-family: 'barmenoroman'; padding-bottom: 50px; font-size: 30px;" class="lead text-center">Our mission is providing high quality and nutritious<br> 
				foods and beverages to consumers.</p>
			<button style="font-family: 'barmenoroman';" type="button" class="btn btn-primary btn-lg">Discover More</button>	
	</div>
	<div class="container">
		<div class="row">
			<div class="col-sm-7 trademarks">
			<h3 style="width: 650px; right: 0; position: absolute; margin: auto;">Discover Our Trademarks</h3>
				<div class="row">
					<div class="col-sm-6">
						<div style="padding-top: 70px;">
							<h4 style="color: black;">Tropicana</h4>
							<p style="font-size: 17px; color: black; ">A range of fresh and long-life juices in<br>
								different flavors.<br>
							<a href="/">Discover more</a></p>
						</div>
					</div>
					<div class="col-sm-6">
						<img style="margin-top: 70px; position: relative;" src="/wp-content/themes/html5blank-stable/img/discoverus_image.png">
					</div>
				</div>
					<div class='row'>
						<div class='col-md-6 slider_fp'>
						  <div class="carousel slide media-carousel" id="recommendations">
						    <div class="carousel-inner">
						      <div class="item  active">
						        <div class="row">
						          <div class="col-md-3">
						            <a href="#"><img alt="" data-src="/wp-content/themes/html5blank-stable/img/front_slider.png"></a>
						          </div>          
						          <div class="col-md-3">
						            <a href="#"><img alt=""  data-src="/wp-content/themes/html5blank-stable/img/front_slider.png"></a>
						          </div>
						          <div class="col-md-3">
						            <a href="#"><img alt=""  data-src="/wp-content/themes/html5blank-stable/img/front_slider.png"></a>
						          </div>
						          <div class="col-md-3">
						            <a href="#"><img alt="" data-src="/wp-content/themes/html5blank-stable/img/front_slider.png"></a>
						          </div>            
						        </div>
						      </div>
						      <div class="item">
						        <div class="row">
						          <div class="col-md-3">
						            <a href="#"><img alt="" data-src="/wp-content/themes/html5blank-stable/img/front_slider.png"></a>
						          </div>          
						          <div class="col-md-3">
						            <a href="#"><img alt=""  data-src="/wp-content/themes/html5blank-stable/img/front_slider.png"></a>
						          </div>
						          <div class="col-md-3">
						            <a href="#"><img alt=""  data-src="/wp-content/themes/html5blank-stable/img/front_slider.png"></a>
						          </div>
						          <div class="col-md-3">
						            <a href="#"><img alt="" data-src="/wp-content/themes/html5blank-stable/img/front_slider.png"></a>
						          </div>            
						        </div>
						      </div>
						    </div>
						    <a data-slide="prev" href="#recommendations" class="left"><span class="glyphicon glyphicon-chevron-left leftSlider"></span></a>
						    <a data-slide="next" href="#recommendations" class="right"><span class="glyphicon glyphicon-chevron-right rightSlider"></span></a>
						  </div>                          
						</div>
					</div>

			</div>
			<div class="col-sm-5 visitus">
				<h3 style="width: 320px; right: 0; position: absolute; margin: auto;">Visit Us</h3>
			</div>
		</div>

		<div class="row">
			<div class="col-sm-6 _socialleft">
				<h3 style="width: 485px; right: 0; position: absolute; margin: auto;">Teeba on Facebook</h3>
			<div class="overlaysocial"><p style="color: white;">Lorem Ipsum adalah text contoh digunakan didalam industri pencetakan dan typesetting.</p></div>		
			</div>
			<div class="col-sm-6 _socialright">
				<h3 style="width: 485px; right: 0; position: absolute; margin: auto;">Teeba on Instagram</h3>
				<div class="overlaysocial"><p style="color: white;">sebuah galeri cetak dan merobakanya menjadi satu buku spesimen. Ia telah bertahan bukan hanya selama lima kurun, tetapi</p></div>
			</div>
		</div>
		<div class="row">
			<div class="col-sm-6 _socialleft">
				<h3 style="width: 485px; right: 0; position: absolute; margin: auto;">Almarai on Facebook</h3>
				<div class="overlaysocial"><p style="color: white;">telah melonjak ke era typesetting elektronik, dengan tiada perubahan ketara. Ia telah dipopularkan pada tahun 1960an dengan penerbitan </p></div>
			</div>
			<div class="col-sm-6 _socialright">
				<h3 style="width: 485px; right: 0; position: absolute; margin: auto;">Almarai on Instagram</h3>
				<div class="overlaysocial"><p style="color: white;">Letraset yang mebawa kangungan Lorem Ipsum, dan lebih terkini dengan sofwer pencetakan desktop seperti</p></div>
			</div>
		</div>
	</div>

	<script type="text/javascript">
		$(document).ready(function() {
		  $('#recommendations').carousel({
		    pause: true,
		    interval: false,
		  });
		});
	</script>

<?php get_footer(); ?>
